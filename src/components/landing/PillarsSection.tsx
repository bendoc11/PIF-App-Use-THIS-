  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: "0 0 40px hsl(5 78% 55% / 0.06), inset 0 1px 0 rgba(255,255,255,0.04)",
      }}
    >
      {/* Top accent */}
      <div className="h-[2px] w-full" style={{ background: "linear-gradient(90deg, hsl(5 78% 55%), hsl(25 95% 53%))" }} />
  ...
  return (
    <PillarCard visible={visible} delay={200} label="DEVELOP">
      ...
      <div className="mt-5 pt-4 border-t border-white/5">
        <p className="font-heading text-2xl text-foreground">500<span className="text-primary">+</span> <span className="text-sm text-muted-foreground">DRILLS</span></p>
      </div>
    </PillarCard>
  );
}
    ...
    <PillarCard visible={visible} delay={400} label="TRACK">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="relative w-12 h-12">
          <svg width={48} height={48} className="rotate-[-90deg]">
            ...
            <circle
              cx={24} cy={24} r={20} fill="none" stroke="hsl(5 78% 55%)" strokeWidth={3}
              strokeDasharray={125.6}
              strokeDashoffset={visible ? 125.6 * 0.15 : 125.6}
              strokeLinecap="round"
              className="transition-all duration-1500 ease-out"
              style={{ transitionDelay: "1000ms" }}
            />
          </svg>
          <span className="absolute inset-0 flex items-center justify-center text-[10px] font-heading text-foreground">85%</span>
        </div>
        <div>
      ...
      <StaggerItem visible={visible} delay={1400}>
        <div className="bg-white/[0.04] rounded-lg p-3 flex items-center justify-between border border-white/5">
          <span className="text-xs font-heading text-foreground">🔥 STREAK</span>
...
function RecruitCard({ visible }: { visible: boolean }) {
  ...
  return (
    <PillarCard visible={visible} delay={600} label="GET RECRUITED">
        ...
        {schools.map((school, i) => (
          <StaggerItem key={school} visible={visible} delay={800 + i * 250}>
            <div className="flex items-center justify-between bg-white/[0.04] rounded-lg px-3 py-2 border border-white/5">
              ...
              <span
                className="text-[10px] font-heading tracking-wider px-2 py-0.5 rounded"
                style={{
                  background: "hsl(142 69% 58% / 0.15)",
                  color: "hsl(142 69% 58%)",
                  opacity: visible ? 1 : 0,
                  transition: "opacity 0.3s ease",
                  transitionDelay: `${1000 + i * 250}ms`,
                }}
              >
                SENT
              </span>
      ...
      {/* Bold stat */}
      <div className="mt-5 pt-4 border-t border-white/5">
        <p className="font-heading text-2xl text-foreground">7,819 <span className="text-sm text-muted-foreground">COACH EMAILS</span></p>
      </div>
    </PillarCard>
  );
}
...
export function PillarsSection() {
  ...
  return (
    <section ref={ref} className="relative py-20 lg:py-32 overflow-hidden" style={{ background: "#060A10" }}>
      {/* Grid texture */}
      <div
